...
def main():
    ...
    clf = GridSearchCV(LinearSVC(), param_grid, n_jobs=-1, cv=5, verbose=1)
    clf.fit(X, y)
    print('score =', clf.best_score_)
    print('params =', repr(clf.best_params_))
    joblib.dump(clf.best_estimator_, 'linear_svc.pkl')
    joblib.dump(scaler, 'linear_svc_scaler.pkl')
    # clf = SVC(kernel=b'rbf', C=2.9394689845511977, gamma=0.0009041745237180573)
